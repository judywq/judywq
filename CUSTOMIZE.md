...
You can create new tools by adding new Markdown files in the [\_tools](_tools/) directory. The easiest way to do this is to copy an existing tool and modify it.
...
This Jekyll theme implements `collections` to let you break up your work into categories. The theme comes with two default collections: `news` and `tools`. Items from the `news` collection are automatically displayed on the home page. Items from the `tools` collection are displayed on a responsive grid on tools page.

You can easily create your own collections, apps, short stories, courses, or whatever your creative work is. To do this, edit the collections in the [\_config.yml](_config.yml) file, create a corresponding folder, and create a landing page for your collection, similar to [\_pages/tools.md](_pages/tools.md).
